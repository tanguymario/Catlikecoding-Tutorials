...
static public class OctahedronSphereCreator 
{
    #region Properties
    ...
    static public Mesh Create(int subdivisions, float radius)
    {
        if (subdivisions < minSubdivisions)
        {
            Debug.LogWarning("Subdivisions are under minimum...\nResetting to : " + minSubdivisions);
            subdivisions = 0;
        }
        else if (subdivisions > maxSubdivisions)
        {
            Debug.LogWarning("Subdivisions are upon minimum...\nResetting to : " + minSubdivisions);
    ...
    static private void Normalize (Vector3[] vertices, Vector3[] normals)
    {
        for (int i = 0; i < vertices.Length; i++)
            normals[i] = vertices[i] = vertices[i].normalized;
    }

    static private void CreateUV (Vector3[] vertices, Vector2[] uv) 
    {
        float previousX = 1f;
    ...
    static private int CreateVertexLine (Vector3 from, Vector3 to, int steps, int v, Vector3[] vertices) 
    {
    ...
    static private int CreateLowerStrip (int steps, int vTop, int vBottom, int t, int[] triangles) 
    {
        for (int i = 1; i < steps; i++) 
        {
            triangles[t++] = vBottom;
    ...
    static private int CreateUpperStrip (int steps, int vTop, int vBottom, int t, int[] triangles) 
    {
        triangles[t++] = vBottom;
        triangles[t++] = vTop - 1;
        triangles[t++] = ++vBottom;
